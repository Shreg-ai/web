...
export function ForgotPasswordForm() {
  ...
  if (sent) {
    return (
      <div className="w-full max-w-sm rounded-xl border border-violet-100 bg-white p-6 text-center shadow-sm">
        <h1 className="mb-2 text-lg font-medium text-violet-950">Check your email</h1>
  ...
  return (
    <form action={handleSubmit} className="w-full max-w-sm rounded-xl border border-violet-100 bg-white p-6 shadow-sm">
      <h1 className="mb-2 text-lg font-medium text-violet-950">Reset your password</h1>
      <p className="mb-4 text-sm text-neutral-500">Enter your email and we&apos;ll send you a reset link.</p>
      <input
        name="email"
        type="email"
        placeholder="Email"
